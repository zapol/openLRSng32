...
uint8_t autopilotRun(uint16_t *inppm, uint16_t *outppm)
{
	...
	static struct pid elevator_pid = {10.0,0.0,0.0,100.0,0.0,0.0};
	static struct pid ailerons_pid = {10.0,0.0,0.0,100.0,0.0,0.0};
...
//		printf("Autopilot ON\n");
		...
		// Elevator - regulate
		error = (float)(inppm[AUTOPILOT_ELEVATOR_CHANNEL]-512.0)/10 + imuData->pitch/M_PI*360;
		output = pid(&elevator_pid, error);
		if(output>512)output = 512;
		if(output<-512)output = -512;
		outppm[AUTOPILOT_ELEVATOR_CHANNEL] = (uint16_t)(output+512);
		printf("Yaw error: %4d, output: %4d ", (int)error, (int)output);
		// Throttle - just copy
		outppm[AUTOPILOT_THROTTLE_CHANNEL] = inppm[AUTOPILOT_THROTTLE_CHANNEL];

		// Ailerons - regulate
		error = (float)(inppm[AUTOPILOT_AILERONS_CHANNEL]-512.0)/10 + imuData->roll/M_PI*360;
		output = pid(&ailerons_pid, error);
		if(output>512)output = 512;
		if(output<-512)output = -512;
		printf("Pitch error: %4d, output: %4d\n", (int)error, (int)output);
		outppm[AUTOPILOT_AILERONS_CHANNEL] = (uint16_t)(output+512);
