...
uint32_t last_autopilot_timeUs = 0;
...
void save_failsafe_values(void) {
	...
	if (FLASH_ErasePage(FLASH_FSWRITE_ADDR) == FLASH_COMPLETE) {
		...
		for (i = 0; i < sizeof(rcvPPM); i += 4) {
			status = FLASH_ProgramWord(FLASH_FSWRITE_ADDR + 4 + i,
					*(uint32_t *) ((char *) &rcvPPM + i));
...
void load_failsafe_values(void) {
	...
	if (*tempb == 0xDEADC0DE) {
		uint8_t i;
		memcpy(&rcvPPM, (char *) (FLASH_FSWRITE_ADDR + 4), sizeof(rcvPPM));
...
//    printf("rxd\r\n");
		...
		if ((rx_buf[0] & 0x3e) == 0x00) { //servo data
			uint8_t i;
			unpackChannels(bind_data.flags & 7, rcvPPM, rx_buf + 1);
//			printf("Received PPM: ");
			for (i = 0; i < PPM_CHANNELS; i++) {
//				printf("%4d ", rcvPPM[i]);
		...
		if (firstpack == 0) {
			...
			for (i = 0; i < MAX_OUTPUTS; i++) {
				if ((rx_config.pinMapping[i] == PINMAP_PPM) & (i == PPM_PIN)) {
					...
					configureLbeepPWM();
				} else if ((rx_config.pinMapping[i] == PINMAP_ANALOG)
						& ((i == ANALOG0_PIN) | (i == ANALOG1_PIN))) {
...
//      printf("lost pkt - slowhop\n");
		...
		if (lostpack) {
			if (rx_config.failsafeDelay && (!failsafeActive)
				...
				linkLossTimeMs = 0;
				last_beacon =
						(timeMs + delayInMsLong(rx_config.beacon_deadtime)) | 1; //beacon activating...
			}
...
//    rfmSetChannel(2, bind_data.hopchannel[RF_channel]);
	...
	if ((timeUs - last_autopilot_timeUs) > AUTONOMY_INTERVAL_US) {
		last_autopilot_timeUs = timeUs;
		autopilotRun(rcvPPM, autoPPM);
		mixerExecute(autoPPM, outPPM);
...
//		printf("\n");
		...
		for (int i = 0; i != PPM_CHANNELS; i++) {
			...
			if (rx_config.RSSIpwm == i) {
				...
				setPWM(i, servoBits2Us(outPPM[rx_config.pinMapping[i]]));
			}
		}
	}
}
...
int main(void) {
	...
	if (bind_data.flags & TELEMETRY_MASK) {
		uartInit(
				((bind_data.flags & TELEMETRY_MASK) == TELEMETRY_FRSKY) ?
						9600 : bind_data.serial_baudrate);
